...
const createRule = __typescript_eslint_utils.ESLintUtils.RuleCreator(docsUrl);
...
const getValue = (node) => {
	switch (node.type) {
		case __typescript_eslint_utils.TSESTree.AST_NODE_TYPES.Identifier: return node.name;
		case __typescript_eslint_utils.TSESTree.AST_NODE_TYPES.Literal: return node.value;
...
function normalizeConfigResolvers(resolvers, sourceFile) {
	...
	for (const nameOrRecordOrObject of resolverArray) if (typeof nameOrRecordOrObject === "string") {
		...
		for (const [name$1, enableOrOptions] of Object.entries(record)) if (typeof enableOrOptions === "boolean") map.set(name$1, {
			name: name$1,
...
function fullResolve(modulePath, sourceFile, settings, context) {
	...
	const cacheKey = sourceDir + "," + childContextHashKey + "," + memoizedHash + "," + modulePath;
	...
	if (Object.hasOwn(settings, "import-x/resolver-next") && settings["import-x/resolver-next"]) {
...
const importDeclaration = (context, node) => {
	if (node.parent && node.parent.type === __typescript_eslint_utils.AST_NODE_TYPES.ImportDeclaration) return node.parent;
...
function removeTypescriptFunctionOverloads(nodes) {
	for (const node of nodes) {
		const declType = node.type === __typescript_eslint_utils.AST_NODE_TYPES.ExportDefaultDeclaration ? node.declaration.type : node.parent?.type;
		if (declType === __typescript_eslint_utils.AST_NODE_TYPES.TSDeclareFunction) nodes.delete(node);
	}
}
/**
...
function testIsAllow(globs, filename, source) {
	...
	return globs.some((glob) => (0, minimatch.minimatch)(filePath, glob) || (0, minimatch.minimatch)(filePath, node_path.default.resolve(glob), { windowsPathsNoEscape: true }));
}
...
const { FileEnumerator } = eslint_use_at_your_own_risk.default;
...
const { AST_NODE_TYPES } = __typescript_eslint_utils.TSESTree;
